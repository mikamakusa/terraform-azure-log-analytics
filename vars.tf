...
variable "storage_insight_account_name" {
  type    = string
  default = null
}
...
variable "workspace" {
  ...
  validation {
    condition = length([
      for a in var.workspace : true if contains(["PerNode", "Premium", "Standard", "Standalone", "Unlimited", "CapacityReservation", "PerGB2018"], a.sku)
    ]) == length(var.workspace)
    error_message = "Possible values are PerNode, Premium, Standard, Standalone, Unlimited, CapacityReservation, and PerGB2018."
  }

  validation {
    condition = length([
      for b in var.workspace : true if contains(["100", "200", "300", "400", "500", "1000", "2000", "5000"], b.reservation_capacity_in_gb_per_day)
    ]) == length(var.workspace)
  ...
  validation {
    condition = length([
      for b in var.workspace : true if contains(["SystemAssigned", "UserAssigned"], b.identity_type)
    ]) == length(var.workspace)
...
variable "workspace_table" {
  ...
  validation {
    condition = length([
      for a in var.workspace_table : true if contains(["Analytics", "Basic"], a.plan)
    ]) == length(var.workspace_table)
    error_message = "Possible values are Analytics and Basic. Defaults to Analytics."
  }

  validation {
    condition = length([
      for b in var.workspace_table : true if b.retention_in_days == 7 || b.retention_in_days >= 30 && b.retention_in_days <= 730
    ]) == length(var.workspace_table)
  ...
  validation {
    condition = length([
      for c in var.workspace_table : true if c.retention_in_days >= 30 && c.retention_in_days <= 4383
    ]) == length(var.workspace_table)
