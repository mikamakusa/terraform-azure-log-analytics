...
data "azurerm_storage_account" "storage_insights" {
  count               = var.storage_insight_account_name ? 1 : 0
  name                = var.storage_insight_account_name
  resource_group_name = data.azurerm_resource_group.this.name
}

data "azurerm_storage_account" "linked_storage" {
  count               = var.linked_storage_account_name ? 1 : 0
  name                = var.linked_storage_account_name
  resource_group_name = data.azurerm_resource_group.this.name
}
